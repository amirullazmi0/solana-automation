...
export class PriceMonitorService {
  ...
  constructor(
    ...
    this.trailingDistancePercent = parseFloat(this.configService.get<string>('TRAILING_DISTANCE_PERCENT', '0.5'));
  }
  ...
  async monitorPrices() {
    ...
    for (const trade of openTrades) {
      try {
        ...
        this.logger.error(`Error monitoring price for ${trade.tokenMint}: ${error.message}`);
      }
    }
  }
  ...
  private async getCurrentPrice(tokenMint: string): Promise<number> {
    try {
      ...
      this.logger.error(`[PriceMonitor] Error fetching price for ${tokenMint}: ${error.message}`);
      return 0;
    }
  }

  private async evaluateTrade(trade: any, currentPrice: number) {
    ...
    if (profitPercent >= 5) {
      if (currentPrice > trade.highestPrice) {
        ...
        this.logger.log(`[Slot ${trade.slotNumber}] New Highest Price: $${currentPrice}. Updated Trailing Stop: $${newTrailingStop}`);
    ...
    if (currentPrice <= trade.trailingStopPrice && profitPercent >= 5) {
      this.logger.log(`[Slot ${trade.slotNumber}] Trailing Take Profit Triggered at $${currentPrice}`);
