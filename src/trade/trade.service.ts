...
export class TradeService implements OnModuleInit {
  ...
  private setupWalletAndConnection() {
    ...
    if (privateKeyString && privateKeyString !== 'YOUR_BASE58_PRIVATE_KEY_HERE') {
      try {
  ...
  async executeSell(tradeId: number, currentPrice: number, isStopLoss: boolean) {
    ...
    const { success } = await this.executeJupiterSwap(trade.tokenMint, WRAPPED_SOL_MINT, tokenAmountToSell, 'SELL');

    if (success) {
      await this.prismaService.trade.update({ where: { id: tradeId }, data: { status: 'CLOSED' } });
      const profit = ((currentPrice - trade.entryPrice) / trade.entryPrice) * 100;
      await this.reportingService.sendSellAlert(trade.tokenMint, currentPrice, profit, isStopLoss);
    }
  }
  ...
  private async executeJupiterSwap(
    ...
    try {
      ...
      const swapResponse = await axios.post(`${baseUrl}/v6/swap`, {
      ...
      const txid = await this.connection.sendRawTransaction(transaction.serialize(), { skipPreflight: true, maxRetries: 3 });
